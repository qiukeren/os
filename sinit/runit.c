...
int main(int argc, const char* const* argv, char* const* envp) {
    ...
    while (pipe(selfpipe) == -1) {
        strerr_warn2(FATAL,
                     "unable to create selfpipe, pausing: ", &strerr_sys);
        sleep(5);
    }
    coe(selfpipe[0]);
    coe(selfpipe[1]);
            ...
            if (st == 0) {
                if ((ttyfd = open("/dev/console", O_RDWR)) != -1) {
                    ...
                    strerr_warn2(WARNING,
                                 "unable to open /dev/console: ", &strerr_sys);
            } else
                setsid();
        ...
        for (;;) {
            ...
            if (child == pid) {
                if (wait_exitcode(wstat) != 0) {
                    ...
                    if (st == 1)
                        /* this is stage 2 */
                        if (wait_crashed(wstat) ||
                            (wait_exitcode(wstat) == 111)) {
                            strerr_warn2(WARNING,
                                         "killing all processes in stage 2...",
                                         0);
                            kill(-pid, 9);
                            sleep(5);
            ...
            if (sigc && (stat(STOPIT, &s) != -1) && (s.st_mode & S_IXUSR)) {
                ...
                if (pid) {
                    /* still there */
                    strerr_warn2(WARNING,
                                 "stage 2 not terminated, sending sigkill...",
                                 0);
                    kill(pid, 9);
