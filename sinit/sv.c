...
#define VERSION "$Id: e076bd67158ea6a9cfe294f85ae1a8f6ed30c4e5 $"
...
int svstatus_get() {
    ...
    switch (r) {
        case 20:
            break;
        case -1:
            warn("unable to read supervise/status");
            return (-1);
        default:
            warnx("unable to read supervise/status: bad format");
            return (-1);
    }
    return (1);
}
...
unsigned int svstatus_print(char* m) {
    ...
    if (pid && svstatus[16])
        outs(", paused");
    if (!pid && (svstatus[17] == 'u'))
        outs(", want up");
    if (pid && (svstatus[17] == 'd'))
        outs(", want down");
    if (pid && svstatus[18])
        outs(", got TERM");
    return (pid ? 1 : 2);
    ...
    while (*a) {
        ...
        switch (*a) {
            case 'x':
                return (0);
            case 'u':
                if (!pid || svstatus[19] != 1)
                    return (0);
                if (!checkscript())
                    return (0);
                break;
            case 'd':
                if (pid || svstatus[19] != 0)
                    return (0);
                break;
            case 'C':
                if (pid)
