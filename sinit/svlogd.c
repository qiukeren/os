...
#define VERSION "$Id: 8c68c739f556270e73f4f5948c0d07cb3c3d7908 $"
        ...
        if ((fd = open_read("state")) == -1) {
            if (errno == error_noent) {
                if ((fd = open_trunc("state")) == -1)
                    fatal2("unable to create empty state for processor",
                           ld->name);
                close(fd);
                if ((fd = open_read("state")) == -1)
...
unsigned int processorstop(struct logdir* ld) {
    ...
    if (unlink(ld->fnsave) == -1)
        strerr_warn5(WARNING, "unable to unlink: ", ld->name, "/", ld->fnsave,
                     0);
    while (rename("newstate", "state") == -1)
        pause2("unable to rename state", ld->name);
    if (verbose)
        strerr_warn5(INFO, "processed: ", ld->name, "/", f, 0);
    while (fchdir(fdwdir) == -1)
        pause1("unable to change to initial working directory");
    return (1);
}
...
unsigned int rotate(struct logdir* ld) {
    ...
    if (ld->size > 0) {
        ...
        if (verbose) {
            ...
            strerr_warn6(INFO, "rename: ", ld->name, "/current ", ld->fnsave,
                         tmp, 0);
        }
...
int buffer_pwrite(int n, char* s, unsigned int len) {
    ...
    while ((i = write((dir + n)->fdcur, s, len)) == -1) {
        if ((errno == ENOSPC) && ((dir + n)->nmin < (dir + n)->nmax)) {
            ...
            if (j > (dir + n)->nmin)
                if (*oldest == '@') {
                    strerr_warn5(WARNING,
                                 "out of disk space, delete: ", (dir + n)->name,
                                 "/", oldest, 0);
                    errno = 0;
                    if (unlink(oldest) == -1) {
                        warn2("unable to unlink oldest logfile",
                              (dir + n)->name);
                        errno = ENOSPC;
                    }
                    while (fchdir(fdwdir) == -1)
...
void logdir_close(struct logdir* ld) {
    if (ld->fddir == -1)
        return;
    if (verbose)
        strerr_warn3(INFO, "close: ", ld->name, 0);
    close(ld->fddir);
    ld->fddir = -1;
    if (ld->fdcur == -1)
        return; /* impossible */
...
unsigned int logdir_open(struct logdir* ld, const char* fn) {
    ...
    if (i != 0) {
        ...
        for (i = 0; i + 1 < sa.len; ++i) {
            ...
            switch (sa.s[i]) {
                ...
                case 's':
                    switch (
                        sa.s[scan_ulong(&sa.s[i + 1], &ld->sizemax) + i + 1]) {
                        case 'm':
                            ld->sizemax *= 1024;
                        case 'k':
                            ld->sizemax *= 1024;
                ...
                case 'U':
                    ld->udponly = 1;
                case 'u':
                    if (!(c = ip4_scan(sa.s + i + 1,
                                       (char*)&ld->udpaddr.sin_addr))) {
                    ...
                    if (sa.s[i + 1 + c] == ':') {
                        ...
                        if (port == 0) {
                            warnx("unable to scan port number",
                                  sa.s + i + c + 2);
                            break;
                        }
                    } else
    ...
    if (verbose) {
        if (i == 0)
            strerr_warn4(INFO, "append: ", ld->name, "/current", 0);
        else
            strerr_warn4(INFO, "new: ", ld->name, "/current", 0);
    }
        ...
        for (i = 0; i < dirn; ++i)
            if (dir[i].fddir != -1) {
                ...
                if (dir[i].udpaddr.sin_port != 0) {
                    ...
                    if (fdudp == -1) {
                        buffer_puts(&dir[i].b,
                                    "warning: no udp socket available: ");
                        if (timestamp)
