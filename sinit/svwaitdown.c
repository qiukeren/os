...
int main(int argc, const char* const* argv) {
            ...
            if (dokill) {
                ...
                if ((fd = open_write("supervise/control")) == -1) {
                    if (errno == error_nodevice) {
                        ...
                        warn(*argv, ": unable to open supervise/control: ",
                             &strerr_sys);
                    continue;
                }
                if (write(fd, "k", 1) != 1)
                    warn(*argv, ": unable to write to supervise/control: ",
                         &strerr_sys);
                else
    ...
    if (fchdir(wdir) == -1)
        strerr_warn2(WARN,
                     "unable to switch to starting directory: ", &strerr_sys);
    close(wdir);
    if (rc > 100)
        rc = 100;
    _exit(rc);
