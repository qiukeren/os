...
#define USAGE_MAIN                                                           \
    ...
    "root] [-C pwd] [-n nice] [-l|-L lock] [-m n] [-d n] [-o n] [-p n] [-f " \
    "n] [-c n] prog"
...
void euidgid(char* user, unsigned int ext) {
    ...
    if (!pathexec_env("GID", bufnum))
        die_nomem();
    bufnum[fmt_ulong(bufnum, ugid.uid)] = 0;
    if (!pathexec_env("UID", bufnum))
        die_nomem();
}
    ...
    for (;;) {
        ...
        if (sa.len) {
            ...
            if (!pathexec_env(d->d_name, sa.s))
                die_nomem();
        } else if (!pathexec_env(d->d_name, 0))
            die_nomem();
    }
    closedir(dir);
    if (fchdir(wdir) == -1)
...
void slock(const char* f, unsigned int d, unsigned int x) {
    ...
    if ((fd = open_append(f)) == -1)
        slock_die("unable to open lock", f, x);
    if (d) {
        if (lock_ex(fd) == -1)
            slock_die("unable to lock", f, x);
        return;
    }
    if (lock_exnb(fd) == -1)
        slock_die("unable to lock", f, x);
...
#else
        if (verbose)
            warn("system does not support RLIMIT_CPU");
#endif
    }
}
...
int main(int argc, const char** argv) {
    ...
    if (str_equal(progname, "setlock"))
        setlock(argc, argv);
    if (str_equal(progname, "softlimit"))
        softlimit(argc, argv);

    while ((opt = getopt(argc, argv,
                         "u:U:b:e:m:d:o:p:f:c:r:t:/:C:n:l:L:vP012V")) != opteof)
        switch (opt) {
            case 'u':
                set_user = (char*)optarg;
                break;
            case 'U':
                env_user = (char*)optarg;
                break;
            case 'b':
                argv0 = (char*)optarg;
                break;
            ...
            case '2':
                ...
                break;
            case 'V':
                strerr_warn1("$Id: d597b680ae59724e54e277ea3b4eada7f55f0839 $",
                             0);
            case '?':
                usage();
        }
    ...
    if (pwd) {
        if (chdir(pwd) == -1)
            fatal2("unable to change directory", pwd);
    }
    if (nicelvl) {
        errno = 0;
        if (nice(nicelvl) == -1)
...
#define USAGE_SOFTLIMIT                                                \
    ...
    "lockbytes] [-m membytes] [-o openfiles] [-p processes] [-r "      \
    "residentbytes] [-s stackbytes] [-t cpusecs] child"
