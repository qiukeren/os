...
void init() {
    ...
    if (GBSH_IS_INTERACTIVE) {
        ...
        setpgid(GBSH_PID, GBSH_PID);  // we make the shell process the new
...
void signalHandler_child(int p) {
     ...
     * not block if a child was cleaned up in another part of the program. */
...
void fileIO(char* args[], char* inputFile, char* outputFile, int option) {
    ...
    int fileDescriptor;  // between 0 and 19, describing the output or input
                         // file
