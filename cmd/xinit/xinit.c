...
#define WAIT_BETWEEN_SIGNALS                        \
    3 /* default time to wait between TERM and KILL \
       */
 ...
 */
static void write_utmp(char* user,    /* name of user */
                       char* id,      /* inittab ID */
                       int pid,       /* PID of process */
                       int type,      /* TYPE of entry */
 ...
 */
void write_utmp_wtmp(char* user, /* name of user */
                     char* id,   /* inittab ID */
                     int pid,    /* PID of process */
                     int type,   /* TYPE of entry */
                     char* line) /* LINE if used. */
...
static void read_inittab(void) {
    ...
    while (done != 1) {
        ...
        LOG_INFO("%s %s %s %s", id, rlevel, action, process);
        ...
        for (ch = family; ch; ch = ch->next) {
            ...
            switch (round) {
                case 0: /* Send TERM signal */
                    if (talk) {
                        initlog(L_CO,
                                "Sending processes configured via /etc/inittab "
                                "the TERM "
                                "signal");
                    }
                    kill(-(ch->pid), SIGTERM);
...
static void fifo_new_level(int level) {
...
#endif
    {
        ...
        if (runlevel == 'U') {
            ...
            read_inittab();
            fail_cancel();
            x_set_proc_title_va("init [%c]", (int)runlevel);
        }
    }
    Write_Runlevel_Log(runlevel);
}
