...
/* nyagetty: modified and made standalone by q66 <q66@chimera-linux.org>
*
* single-file version, public domain like upstream version,
* 0BSD where public domain is not legally valid
*/
#define _PATH_RUNSTATEDIR "/run"
#define _PATH_SYSCONFSTATICDIR ""
#define PACKAGE_VERSION "VERSION-1.1"
...
/* auxiliary paths, from pathnames.h */
...
#ifndef _PATH_LOGINDEFS
#define _PATH_LOGINDEFS "/etc/login.defs"
#endif

/*
* this comes from all-io.h
*/
...
static inline int write_all(int fd, const void *buf, size_t count)
{
   while (count) {
       ...
       if (errno == EAGAIN) {	/* Try later, *sigh* */
           struct timespec wt = {.tv_sec = 0, .tv_nsec = 250000000L};
           nanosleep(&wt, NULL);
       }
   }
   return 0;
...
/*
* this comes from color-names.c/h
*/
...
static const char *color_sequence_from_colorname(const char *str)
{
   ...
   res = bsearch(&key, basic_schemes, sizeof(basic_schemes) / sizeof(basic_schemes[0]),
...
static uint64_t str2unum_or_err(const char *str, int base, const char *errmesg, uint64_t up)
{
   uint64_t num = 0;
   int rc;
...
static int get_terminal_stdfd(void)
{
...
static int get_terminal_name(const char **path,
                            const char **name,
                            const char **number)
{
   const char *tty;
...
/* parse the one value we need from login.defs, it's a hacky parse but
* does pretty much the same job as the upstream util-linux code
*/
static int get_login_plain_prompt(void)
{
   FILE *f = fopen(_PATH_LOGINDEFS, "r");
   if (!f) return 0;
   ...
   while ((nread = getline(&line, &len, f)) != -1)
   {
       ...
       /* check if this matches the value we want */
       if (strncmp(ln, "LOGIN_PLAIN_PROMPT", sizeof("LOGIN_PLAIN_PROMPT") - 1))
           continue;
       /* if it matches, try getting the value */
       ln += sizeof("LOGIN_PLAIN_PROMPT") - 1;
       while (*ln && isspace(*ln)) ++ln;
       /* check the value */
       if (strncmp(ln, "yes", 3)) goto do_free;
       /* check trailing part */
       ln += 3;
       while (*ln && isspace(*ln)) ++ln;
       result = (*ln == '\0');
...
/* FROM THIS POINT IS ORIGINAL AGETTY SOURCE (only slightly modified) */
...
#  endif
#else
# error "this should not be encountered"
#endif
...
static int wait_for_term_input(int fd)
{
   ...
   while (1) {
       ...
       if (inotify_fd >= 0) {
           ...
           nfds = (inotify_fd > nfds) ? inotify_fd : nfds;
       }
       if (netlink_fd >= 0) {
           FD_SET(netlink_fd, &rfds);
           nfds = (netlink_fd > nfds) ? netlink_fd : nfds;
       }
