    ...
    if (access_letter != NULL) {
        while (*access_letter) {
            if (*first) {
                ...
                *accessor_length +=
                    fprintf(destination, ", -%c", *access_letter);
            }
            ++access_letter;
        }
    }
...
static void cag_option_print_name(const cag_option* option,
    ...
    if (option->access_name != NULL) {
        if (*first) {
            *accessor_length +=
                fprintf(destination, "--%s", option->access_name);
        } else {
            *accessor_length +=
                fprintf(destination, ", --%s", option->access_name);
        }
...
static const cag_option* cag_option_find_by_name(cag_option_context* context,
    ...
    for (i = 0; i < context->option_count; ++i) {
        ...
        // Try to compare the name of the access name. We can use the name_size
        // or this comparison, since we are guaranteed to have null-terminated
        // access names.
...
static const cag_option* cag_option_find_by_letter(cag_option_context* context,
    ...
    for (i = 0; i < context->option_count; ++i) {
        ...
        // Verify whether this option has the access letter in it's access
        // letter string. If it does, then this is our option.
...
static void cag_option_parse_value(cag_option_context* context,
                                   const cag_option* option,
                                   char** c) {
    // And now let's check whether this option is supposed to have a value,
    // which is the case if there is a value name set. The value can be either
    // submitted with a '=' sign or a space, which means we would have to jump
    // over to the next argv index. This is somewhat ugly, but we do it to
    // behave the same as the other option parsers.
    if (option->value_name != NULL) {
        if (**c == '=') {
            context->value = ++(*c);
        } else {
            // If the next index is larger or equal to the argument count, then
            // the parameter for this option is missing. The user will know
            // about this, since the value pointer of the context will be NULL
            // because we don't set it here in that case.
            if (context->argc > context->index + 1) {
                // We consider this argv to be the value, no matter what the
                // contents are.
                ++context->index;
...
static void cag_option_parse_access_name(cag_option_context* context,
    ...
    // Now we need to extract the access name, which is any symbol up to a '='
    // or a '\0'.
    n = *c;
    while (**c && **c != '=') {
        ++*c;
    ...
    // Now this will obviously always be true, but we are paranoid. Sometimes.
    // It doesn't hurt to check.
    assert(*c >= n);

    // Figure out which option this name belongs to. This might return NULL if
    // the name is not registered, which means the user supplied an unknown
    // option. In that case we return true to indicate that we finished with
    // this option. We have to skip the value parsing since we don't know
    // whether the user thinks this option has one or not. Since we don't set
    // any identifier specifically, it will remain '?' within the context.
    option = cag_option_find_by_name(context, n, (size_t)(*c - n));
    if (option == NULL) {
        // Since this option is invalid, we will move on to the next index.
...
static void cag_option_shift(cag_option_context* context,
    ...
    // Lets loop through the option strings first, which we will move towards
    // the beginning.
    ...
    // The new index will be before all non-option values, in such a way that
    // they all will be moved again in the next fetch call.
...
static int cag_option_find_next(cag_option_context* context) {
    ...
    // Check whether it is a '-'. We need to find the next option - and an
    // option always starts with a '-'. If there is a string "-\0", we don't
    // consider it as an option neither.
    while (!cag_option_is_argument_string(c)) {
        c = context->argv[++next_option_index];
        if (c == NULL) {
            // We reached the end and did not find any argument anymore. Let's
            // tell our caller that we reached the end.
            return -1;
        }
    }
...
bool cag_option_fetch(cag_option_context* context) {
    ...
    // index as well as the new index. In the end we will move the option junk
    // to the beginning, so that non option arguments can be read.
    ...
    if (*c == '-') {
        ...
        // This might be a double "--" which indicates the end of options. If
        // this is the case, we will not move to the next index. That ensures
        // that another call to the fetch function will not skip the "--".
        if (*c == '\0') {
            context->forced_end = true;
        } else {
            // We parse now the access name. All information about it will be
