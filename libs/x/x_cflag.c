...
static unsigned term_columns(FILE* f) {
#if HAVE_TIOCGWINZS
    struct winsize ws;
    int fd = fileno(f);
...
static bool findent(FILE* out, unsigned rcol, const char* text) {
    if (rcol < 20)
        return false;
...
static inline bool is_long_flag(const char* s) {
    return (s[0] == '-' && s[1] == '-' && s[2] != '\0');
}

static inline bool is_short_flag(const char* s) {
    return (s[0] == '-' && s[1] != '\0' && s[2] == '\0');
}

static inline bool is_negated(const char* s) {
    return strncmp("--no-", s, 5) == 0;
}

static const struct cflag* find_short(const struct cflag specs[], int letter) {
    for (unsigned i = 0; specs[i].name != NULL || specs[i].letter != '\0';
         ++i) {
        if (specs[i].letter == '\0')
            continue;
...
static const struct cflag* find_long(const struct cflag specs[],
                                     const char* name) {
    for (unsigned i = 0; specs[i].name != NULL || specs[i].letter != '\0';
         ++i) {
        if (!specs[i].name)
...
static inline bool needs_arg(const struct cflag* spec) {
...
int cflag_parse(const struct cflag specs[], int* pargc, char*** pargv) {
    assert(specs);
    assert(pargc);
    assert(pargv);
...
enum cflag_status cflag_bool(const struct cflag* spec, const char* arg) {
    (void)arg;
...
enum cflag_status cflag_int(const struct cflag* spec, const char* arg) {
    if (!spec)
        return CFLAG_NEEDS_ARG;

    return (sscanf(arg, "%d", (int*)spec->data) == 1) ? CFLAG_OK
                                                      : CFLAG_BAD_FORMAT;
}

enum cflag_status cflag_uint(const struct cflag* spec, const char* arg) {
    if (!spec)
        return CFLAG_NEEDS_ARG;

    return (sscanf(arg, "%u", (unsigned*)spec->data) == 1) ? CFLAG_OK
                                                           : CFLAG_BAD_FORMAT;
}

enum cflag_status cflag_float(const struct cflag* spec, const char* arg) {
    if (!spec)
        return CFLAG_NEEDS_ARG;
...
enum cflag_status cflag_double(const struct cflag* spec, const char* arg) {
    if (!spec)
        return CFLAG_NEEDS_ARG;
...
enum cflag_status cflag_string(const struct cflag* spec, const char* arg) {
    if (!spec)
        return CFLAG_NEEDS_ARG;
...
enum cflag_status cflag_bytes(const struct cflag* spec, const char* arg) {
    if (!spec)
        return CFLAG_NEEDS_ARG;
...
enum cflag_status cflag_timei(const struct cflag* spec, const char* arg) {
    if (!spec)
        return CFLAG_NEEDS_ARG;
...
enum cflag_status cflag_help(const struct cflag* spec, const char* arg) {
    (void)spec;
    (void)arg;
